...
// 데이터베이스 연결 설정
const getDbConfig = () => {
  // Firebase Config에서 설정 가져오기 시도
  try {
    const fbConfig = functions.config();
    
    return {
      host: fbConfig.database?.host || process.env.DB_HOST || '211.248.190.46',
      user: fbConfig.database?.user || process.env.DB_USER || 'hermes',
      password: fbConfig.database?.password || process.env.DB_PASSWORD || 'mcygicng!022',
      database: fbConfig.database?.name || process.env.DB_NAME || 'hermes',
    ...
    console.warn('Firebase Config 가져오기 실패, 기본값 사용:', error.message);
