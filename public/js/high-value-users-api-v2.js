...
async function loadUserData() {
  ...
  try {
    ...
    // 정렬 및 페이지네이션 옵션 (HTML 페이지에서 정의된 전역 변수 사용)
    const sortParams = { 
      sortBy: (typeof sortField !== 'undefined') ? sortField : 'netBet', 
      sortOrder: (typeof sortOrder !== 'undefined') ? sortOrder : 'desc' 
    };
    const paginationParams = { 
      page: (typeof currentPage !== 'undefined') ? currentPage : 1, 
      limit: (typeof pageSize !== 'undefined') ? pageSize : 20 
    };
    
    // API 경로 선택
    let endpoint = API_CONFIG.endpoints.active;
    let params = {
      ...paginationParams,
      ...sortParams,
    ...
    if (statusFilter === 'dormant') {
      endpoint = API_CONFIG.endpoints.dormant;
    } else if (statusFilter === 'all') {
      // 'all' 옵션은 현재 active API로 처리 (추후 통합 API 개발 시 변경)
      endpoint = API_CONFIG.endpoints.active;
    }
